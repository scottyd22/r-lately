...
server <- function(input, output, session) {
  ...
  # load data ----
  observe({
    rstudio$d = gather_rstudio_blogs(30)
    ropensci$d = gather_ropensci_blogs(30)
  ...
  output$reference = renderUI({
    ...
    HTML(paste0(
      ...
      '<br><br>',
      'When the app is launched, various R-inspired blogs are scraped and CRAN events searched for the most up-to-date information.',
      '<br><br>',
  ...
  # rstudio data ----
  rstudio = reactiveValues(d = NULL)
  
  # rstudio counter ----
  rstudio_counter = reactiveVal(1)
  
  # rstudio prev,next buttons and actions ----
  output$rstudio_prev = renderUI({
      actionButton('rstudioPrev', label = NULL, icon = icon(name = 'arrow-left', lib = 'font-awesome'),
                   class = 'button')
  })
  
  output$rstudio_next = renderUI({
      actionButton('rstudioNext', label = NULL, icon = icon(name = 'arrow-right', lib = 'font-awesome'),
                   class = 'button')
  })
  
  observeEvent(input$rstudioPrev,{
    # if at the beginning, cycle to the last entry
    if(rstudio_counter() == 1) {
      rstudio_counter(nrow(rstudio$d))
    # otherwise just decrement by 1
    } else {
      rstudio_counter(rstudio_counter() - 1) 
    }
  })
  
  observeEvent(input$rstudioNext,{
    # if at the end, cycle to the first entry
    if(rstudio_counter() == nrow(rstudio$d)) {
      rstudio_counter(1)
  ...
  # rstudio description ----
  output$rstudio_description = renderUI({
    
    HTML('<div style="margin-left: 40px; margin-right: 40px;">
                <b>Check out the latest from RStudio!</b>
                <br><br>
                Content published in the last <b>30 days</b> is gathered from the
                <i><a href="https://www.rstudio.com/rstudio/" target="_blank" style="color: #337ab7;">RStudio Blog &nbsp;<i class="fas fa-external-link-alt"></i></a></i>, 
                
                <i><a href="https://rviews.rstudio.com/" target="_blank" style="color: #337ab7;">R Views Blog &nbsp;<i class="fas fa-external-link-alt"></i></a></i>, and 
                
                <i><a href="https://www.tidyverse.org/rstudio/" target="_blank" style="color: #337ab7;">Tidyverse Blog &nbsp;<i class="fas fa-external-link-alt"></i></a></i>. 
                </div>')
  ...
  # rstudio cards ----
  output$rstudio_cards = renderUI({
    lapply(rstudio_counter(), generate_blog_card, blog = rstudio$d)
    })
  
  # rstudio circles ----
  output$rstudio_circles = renderUI({
    HTML(paste0(
      '<div style="margin-top: 20px;">',
      lapply(1:nrow(rstudio$d), generate_circles, counter = rstudio_counter()) |> paste(collapse = ' '),
      '</div>'
      ))
  })
